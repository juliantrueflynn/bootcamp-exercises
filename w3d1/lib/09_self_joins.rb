...
def craiglockhart_and_tollcross
  ...
  execute(<<-SQL)
    ...
    FROM
      routes start_routes
    JOIN
      routes end_routes ON start_routes.company = end_routes.company AND start_routes.num = end_routes.num
    WHERE
      start_routes.stop_id = 53 AND end_routes.stop_id = 230
  SQL
end
...
def start_at_craiglockhart
  ...
  execute(<<-SQL)
    SELECT DISTINCT
      stops.name, a.company, a.num
    FROM
      stops
    JOIN
      routes a ON stops.id = a.stop_id
    WHERE
      a.stop_id = 53
  SQL
end
